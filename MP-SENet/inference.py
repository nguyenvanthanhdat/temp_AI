...
def inference(a):
    ...
    with torch.no_grad():
        for i, index in enumerate(tqdm(test_indexes)):
            noisy_wav, _ = librosa.load(os.path.join(a.input_noisy_wavs_dir, index+'.wav'), sr=h.sampling_rate)
            # noisy_wav, _ = librosa.load(os.path.join(a.input_noisy_wavs_dir, '00001'+'.wav'), sr=h.sampling_rate)
            ...
            print(os.path.join(a.output_dir, index+'.wav'))
...
def main():
    ...
    parser.add_argument('--input_clean_wavs_dir', default='MP-SENet/VoiceBank+DEMAND/wavs_clean')
    # parser.add_argument('--input_noisy_wavs_dir', default='MP-SENet/VoiceBank+DEMAND/wavs_noisy')
    parser.add_argument('--input_noisy_wavs_dir', default='audio_input')
    parser.add_argument('--input_test_file', default='MP-SENet/VoiceBank+DEMAND/test.txt')
    parser.add_argument('--output_dir', default='audio_output_MP-SENet')
    parser.add_argument('--checkpoint_file', default="MP-SENet/best_ckpt/g_best")
    a = parser.parse_args()
